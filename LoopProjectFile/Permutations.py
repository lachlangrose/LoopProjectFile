...
def checkBrokenRules(lst,rules):
    ...
    for rule in rules:
        if rule[0] in lst and rule[1] in lst:
            if (lst.index(rule[0]) > lst.index(rule[1])): return True
    return False
...
def perm(baseNumber,numberLeft=-1,lst=[],rules=[],current=[],isInvalid=checkBrokenRules,returnList=True):
    ...
    if len(lst) == 0:
        ...
        if numberLeft == -1: numberLeft = len(lst)
...
def CalcPermutation(eventList):
    # Define base case
    if (len(eventList) <= 1): return 1,1,0
    ...
    for x in range(0, len(eventList)):
        if (eventList[x].minAge > maxAge):
            ...
            if (eventList[x].maxAge > maxAge): maxAge = eventList[x].maxAge
    ...
    if (len(workingList) == 2):
        if (workingList[1].minAge < workingList[0].maxAge): permValue = 2
        else: permValue = 1
    if (len(workingList) > 2):
